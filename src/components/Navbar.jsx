        ...
        {dropdownOpen && (
          <div className="absolute top-full mt-3 w-48 -translate-x-1/2 left-1/2 bg-white/95 dark:bg-zinc-800/95 backdrop-blur-md shadow-lg rounded-xl p-2 z-50 border border-gray-200 dark:border-zinc-700">
            {moreItems.map(({ name, path }) => {
              const itemClasses = "block w-full text-left text-sm text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-zinc-700 rounded-md px-3 py-2 transition-colors";
              const closeMenus = () => {
      ...
      <div className="w-full max-w-4xl backdrop-blur-md bg-white/80 dark:bg-zinc-900/70 border border-gray-200 dark:border-zinc-700/80 rounded-full px-5 py-3 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-4">
          <a href="https://linkedin.com" target="_blank" rel="noreferrer" aria-label="LinkedIn Profile">
            {/* --- FIX: Changed hover color for light mode --- */}
            <Linkedin size={18} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors" />
          </a>
          <a href="https://x.com" target="_blank" rel="noreferrer" aria-label="Twitter Profile">
            {/* --- FIX: Changed hover color for light mode --- */}
            <X size={18} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors" />
          </a>
          <a href="https://github.com" target="_blank" rel="noreferrer" aria-label="GitHub Profile">
            {/* --- FIX: Changed hover color for light mode --- */}
            <Github size={18} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors" />
          </a>
          ...
          <button onClick={toggleTheme} aria-label={`Activate ${theme === 'dark' ? 'light' : 'dark'} mode`}>
            {theme === "dark" ? <Sun size={20} className="text-gray-400 hover:text-white transition-colors" /> : <Moon size={20} className="text-gray-500 hover:text-gray-900 transition-colors" />}
          </button>
        </div>
          ...
          <nav className="flex flex-col gap-6 items-center">
            ...
            <div className="w-full border-t border-zinc-200 dark:border-zinc-700 pt-6 mt-4 flex justify-between items-center">
              <div className="flex gap-6">
                <a href="https://linkedin.com" target="_blank" rel="noreferrer" aria-label="LinkedIn Profile">
                    {/* --- FIX: Added consistent hover effect --- */}
                    <Linkedin size={20} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors"/>
                </a>
                <a href="https://x.com" target="_blank" rel="noreferrer" aria-label="Twitter Profile">
                    {/* --- FIX: Added consistent hover effect --- */}
                    <X size={20} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors"/>
                </a>
                <a href="https://github.com" target="_blank" rel="noreferrer" aria-label="GitHub Profile">
                    {/* --- FIX: Added consistent hover effect --- */}
                    <Github size={20} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors"/>
                </a>
              </div>
              <button onClick={toggleTheme} aria-label={`Activate ${theme === 'dark' ? 'light' : 'dark'} mode`}>
